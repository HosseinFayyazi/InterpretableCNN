...
class KernelGammaConv(nn.Module):
    def __init__(self, N_filt, Filt_dim, fs):
        super(KernelGammaConv, self).__init__()
        self.gpu = torch.cuda.is_available()
        # Mel Initialization of the filterbanks
        # mel_freqs = SignalUtils.get_mel_freqs(N_filt, fs)
        # mel_freqs = SignalUtils.get_erb_freqs(N_filt, fs//2)[::-1]
        ...
        self.amplitude_list = nn.Parameter(torch.ones(N_filt))
        # self.order_list = nn.Parameter(4 * torch.ones(N_filt))
    ...
    def forward(self, x):
        ...
        # orders = torch.abs(self.order_list)
        ...
        for i in range(self.N_filt):
            ...
            order = 4  # orders[i].float()
            impulse_response = SignalUtils.kernel_gamma(amp, f1, f2, order, t)
